...
@available(iOS 9.0, *)
public class ContactsServicePlugin: NSObject, FlutterPlugin, CNContactViewControllerDelegate, CNContactPickerDelegate {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        let instance = ContactsServicePlugin(rootViewController)
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "getAvatar":
            let arguments = call.arguments as! [String:Any]
            let contact = arguments["contact"] as! [String:Any]
            let photoHighResolution = arguments["photoHighResolution"] as? Bool ?? false
            getAvatar(contact: contact, photoHighResolution: photoHighResolution, result: result)
        default:
    ...
    @objc func cancelContactForm() {
        if let result = self.result {
            ...
            result(ContactsServicePlugin.FORM_OPERATION_CANCELED)
            self.result = nil
        }
    }
    ...
    // Change the method signature to mark result as @escaping
         ...
         do {
            // Check to make sure dictionary has an identifier
             guard let identifier = contact["identifier"] as? String else{
                 result(ContactsServicePlugin.FORM_COULD_NOT_BE_OPEN)
                 return nil;
             }
            ...
            // Try to fetch the contact - this could fail if the contact doesn't exist
            guard let cnContact = try? store.unifiedContact(withIdentifier: identifier, keysToFetch: keysToFetch) else {
                NSLog("Contact with identifier \(identifier) not found")
                result(ContactsServicePlugin.FORM_COULD_NOT_BE_OPEN)
                return nil
            }
            ...
            // Store the result for later use in the delegate callback
            self.result = result
            ...
            NSLog("Error opening contact: \(error.localizedDescription)")
            result(ContactsServicePlugin.FORM_COULD_NOT_BE_OPEN)
            return nil
         }
     }
...
func getAvatar(contact: [String:Any], photoHighResolution: Bool, result: @escaping FlutterResult) {
    let store = CNContactStore()
    do {
        guard let identifier = contact["identifier"] as? String else {
            result(nil)
        ...
        let keys: [CNKeyDescriptor] = photoHighResolution ? 
            [CNContactImageDataKey as CNKeyDescriptor] :
            [CNContactThumbnailImageDataKey as CNKeyDescriptor]
        
        let cnContact = try store.unifiedContact(withIdentifier: identifier, keysToFetch: keys)
        
        if photoHighResolution {
            result(cnContact.imageData != nil ? FlutterStandardTypedData(bytes: cnContact.imageData!) : nil)
        } else {
            result(cnContact.thumbnailImageData != nil ? FlutterStandardTypedData(bytes: cnContact.thumbnailImageData!) : nil)
        }
    } catch {
